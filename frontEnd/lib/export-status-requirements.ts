/**
 * Required data before advancing export bulking status (forward-only).
 * Mirrors backend export-status-requirements (keep in sync).
 */
...
export type ExportBulkingListForRequirements = {
  ...
  cargo_count?: number;
  shipping_instructions?: SiForRequirements[] | ShippingInstruction[];
};

function resolveEffectiveTotalQuantity(data: ExportBulkingListForRequirements): number | null {
...
/** Fields that must be satisfied while in `current` before moving to `next`. */
...
function statusIndex(status: string): number {
  return EXPORT_BULKING_STATUSES.indexOf(status as ExportBulkingStatus);
}

export function getNextExportBulkingStatus(current: string): ExportBulkingStatus | null {
...
function hasCargoLines(data: ExportBulkingListForRequirements): boolean {
...
function fieldSatisfied(key: string, data: ExportBulkingListForRequirements): boolean {
  const sis = (data.shipping_instructions ?? []) as SiForRequirements[];
...
export function getMissingRequirementsForAdvance(
  data: ExportBulkingListForRequirements,
): string[] {
...
export function getMissingRequirementLabels(data: ExportBulkingListForRequirements): string[] {
  return getMissingRequirementsForAdvance(data).map(
    (k) => EXPORT_STATUS_FIELD_LABELS[k] ?? k,
  );
}

export function canAdvanceExportBulkingStatus(data: ExportBulkingListForRequirements): boolean {
  return getNextExportBulkingStatus(data.current_status) != null &&
    getMissingRequirementsForAdvance(data).length === 0;
}

/** Completion requirements for the terminal stage (CASE_OFF — no further advance). */
export function getMissingVoyageCompletionLabels(data: ExportBulkingListForRequirements): string[] {
  if (data.current_status !== "CASE_OFF") return [];
  return getMissingRequirementLabels(data);
}

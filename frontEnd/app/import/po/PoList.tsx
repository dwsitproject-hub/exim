      ...
      case "po_number":
        return (
          <TableCell key={column.id}>
            <Link href={`/import/po/${row.id}`} className={styles.cellLink} onClick={(e) => e.stopPropagation()}>
              {row.po_number}
            </Link>
          </TableCell>
        );
        ...
        return (
          <TableCell key={column.id}>
            <div className={styles.actionCell} onClick={(e) => e.stopPropagation()}>
              <Link href={`/import/po/${row.id}`} className={styles.actionBtn}>
                View
              </Link>
              {can(user, "UPDATE_PO_INTAKE") && (
                <Link href={`/import/po/${row.id}/edit`} className={styles.actionBtn}>
                  Edit
                </Link>
              )}
            </div>
